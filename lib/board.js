...
var changeStatus = function(ticket, change, newStatus, oldParent, newParent) {
  if (ticket.status !== newStatus) {
    ...
    oldParent= _.without(oldParent, ticket);
    newParent.push(ticket);
  }
};
...
var board = {
  ...
  render: function() {
    this.ticketsByStatus = this.app.data.statuses.map(function(status, index) {
  ...
  transition: function(e) {
    ...
    oldParent = this.tickets.byStatus[ticket.statusIndex];
    newParent = this.tickets.byStatus[e.newStatus];
    tickets = newParent;
    newStatus = this.app.data.statuses[e.newStatus].value;

    changeStatus(ticket, change, newStatus, oldParent, newParent);
    changeAssignee(ticket, change, this.app.currentUser().id());
